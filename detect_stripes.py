...
def DrawFinalResistor(Stripes, SizeFinal):
    ...
    for i in range(Stripes.shape[0]):
        cv2.rectangle(ResFinalBack, (i * StripeWidth, 0), (((i + 1) * StripeWidth) - 1, SizeFinal[1]),
                      np.float64(Stripes[i]), -1)
...
def FindStripesColor(ImageData, Debug, Test):
    ...
    gradGrayX = cv2.Sobel(DRGray, cv2.CV_32F, 1, 0, -1)
    gradGrayY = cv2.Sobel(DRGray, cv2.CV_32F, 0, 1, -1)
...
ResistorsInImage = FindInImage('pic3.jpg')
